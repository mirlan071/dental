...
import { paymentMethodLabels } from "../payments/payment-methods";
...
> = {
  SCHEDULED: [
    { status: "IN_PROGRESS", label: "Начать приём" },
    { status: "CANCELLED", label: "Отменить", variant: "danger" },
    { status: "NO_SHOW", label: "Не пришёл", variant: "secondary" },
  ],
  IN_PROGRESS: [
...
export function AppointmentDetailsPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => api<Appointment>(`/api/appointments/${id}`),
    enabled: Boolean(id),
  });
  const statusMutation = useMutation({
    mutationFn: (status: AppointmentStatus) =>
      api<Appointment>(`/api/appointments/${id}/status`, {
        method: "PATCH",
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          <Card className="grid gap-5 p-5 sm:grid-cols-3">
            <Info icon={Users} label="Пациент" value={item.patientName} />
            <Info icon={UserRound} label="Врач" value={item.doctorName} />
            <Info
              icon={CalendarClock}
              label="Время"
        ...
        <Card className="h-fit p-5 xl:sticky xl:top-24">
          <h2 className="font-semibold text-slate-900">Расчёт</h2>
          <div className="mt-5 space-y-3 text-sm">
            <SummaryRow
              label="Стоимость услуг"
              value={formatMoney(item.servicesTotal)}
            />
            <SummaryRow label="Оплачено" value={formatMoney(item.paidTotal)} />
          ...
          {item.remainingBalance > 0 && (
            <div className="mt-5 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
              Ожидается оплата {formatMoney(item.remainingBalance)}
            </div>
          )}
        </Card>
      </div>
...
function PaymentIcon({ method }: { method: PaymentMethod }) {
  const Icon =
    method === "CASH" ? Banknote : method === "CARD" ? CreditCard : QrCode;
  return (
...
function PaymentDialog({
  ...
  const queryClient = useQueryClient();
  const [amount, setAmount] = useState(String(appointment.remainingBalance));
  const [method, setMethod] = useState<PaymentMethod>("CASH");
  const mutation = useMutation({
    mutationFn: (input: PaymentInput) =>
      api("/api/payments", { method: "POST", body: JSON.stringify(input) }),
    onSuccess: async () => {
      onOpenChange(false);
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: ["appointment", String(appointment.id)],
        }),
        queryClient.invalidateQueries({ queryKey: ["appointments"] }),
        queryClient.invalidateQueries({ queryKey: ["dashboard"] }),
      ]);
  ...
  function submit(e: FormEvent) {
    ...
    if (value <= 0 || value > appointment.remainingBalance) return;
    mutation.mutate({
      ...
      <form onSubmit={submit}>
        <div className="mb-5 rounded-lg bg-brand-50 p-4">
          ...
          <p className="mt-1 text-2xl font-semibold text-brand-800">
            {formatMoney(appointment.remainingBalance)}
          </p>
        </div>
        <label className="block text-sm font-medium text-slate-700">
          ...
          <Input
            ...
            min="0.01"
            step="0.01"
            max={appointment.remainingBalance}
            value={amount}
        ...
        <label className="mt-4 block text-sm font-medium text-slate-700">
          Способ оплаты
          <select
            className="mt-2 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus:border-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-100"
            value={method}
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button
            disabled={
              mutation.isPending ||
              Number(amount) <= 0 ||
              Number(amount) > appointment.remainingBalance
            }
          >
            {mutation.isPending ? "Сохраняем…" : "Добавить оплату"}
          </Button>
        </div>
      </form>
    </Dialog>
