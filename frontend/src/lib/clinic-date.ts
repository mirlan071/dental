...
export function shiftClinicDate(value: string, days: number) {
...
export function clinicDayRange(date = clinicToday()) {
  return {
    from: `${date}T00:00:00${CLINIC_OFFSET}`,
    to: `${shiftClinicDate(date, 1)}T00:00:00${CLINIC_OFFSET}`,
  };
}
...
export function clinicPeriodRange(period: DashboardPeriod) {
  ...
  if (period === "month") {
    ...
    return {
      from: `${dateString(year, month, 1)}T00:00:00${CLINIC_OFFSET}`,
      to: `${nextMonth}T00:00:00${CLINIC_OFFSET}`,
    };
  }
  ...
  const weekday = Number(
    new Intl.DateTimeFormat("en-US", {
      ...
      ? 7
      : localMidday.getUTCDay() || 7,
  );
  const monday = shiftClinicDate(today, -(weekday - 1));
  return {
    from: `${monday}T00:00:00${CLINIC_OFFSET}`,
    to: `${shiftClinicDate(monday, 7)}T00:00:00${CLINIC_OFFSET}`,
  };
}

export function formatClinicDay(value: string, mobile = false) {
