...
public interface PaymentRepository extends JpaRepository<Payment,Long>{ List<Payment> findByAppointmentIdOrderByPaidAt(Long appointmentId); List<Payment> findByPaidAtBetween(OffsetDateTime from,OffsetDateTime to); List<Payment> findByAppointmentDoctorIdAndPaidAtBetween(Long doctorId,OffsetDateTime from,OffsetDateTime to); }
