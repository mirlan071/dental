...
import com.dentalcrm.common.NotFoundException; import org.springframework.stereotype.Service; import org.springframework.transaction.annotation.Transactional; import java.util.List; import static com.dentalcrm.patient.PatientDtos.*;
...
public class PatientService {private final PatientRepository repo;public PatientService(PatientRepository r){repo=r;}
 ...
 public Patient get(Long id){return repo.findById(id).orElseThrow(()->new NotFoundException("Patient not found: "+id));}
 private void apply(Patient p,PatientRequest r){p.setFullName(r.fullName().trim());p.setPhone(r.phone().trim());p.setBirthDate(r.birthDate());p.setNotes(r.notes());}
